...
#sub payment_authorizenet
#{
#	my (@arg) = @_;
#	my $param = ref($arg[0]) ? $arg[0] : {@arg};
#	my $pid = proc_id('authorizenet');
#	my $src = account_id('payment_authorizenet');
#	my $dst = new Cyber::Account($param->{'acct_dst'});
#	$dst->valid_account();
#	my $amount = $param->{'amount'};
#	my $cfg = $::config{'authorizenet'};
#	my $lock;
#	eval {
#		$lock = Cyber::Record::create('payment_lock', {
#			'account' => $param->{'acct_dst'},
#		});
#	};
#	if ($@) # Transaction in progress
#	{
#		return undef;
#	}
#	my $attempt = Cyber::Record::create('payment_attempt', {
#		'amount' => $param->{'amount'},
#		'ts' => strftime("%F %T", localtime(time())),
#		'accepted' => 0,
#		'card_id' => $param->{'card_id'},
...
#			'amount' => $param->{'amount'},
#			'invoice_number' => $attempt->{'id'},
#			'customer_id' => (new Cyber::Record($param->{'acct_dst'}))->data('owner')->{'id'},
...
#			'state' => $param->{'state'},
#			'zip' => $param->{'zip'},
#			'card_number' => $param->{'cc_num'},
#			'expiration' => $param->{'cc_expm'}. '/'. $param->{'cc_expy'},
#		);
...
#		if ($tx->is_success())
#		{
#			my $prec = Cyber::Record::create('payment', {
#				'amount' => $param->{'amount'},
#				'ts' => strftime("%F %T", localtime(time())),
#				'card_id' => $param->{'card_id'},
#				'proc_id' => $pid,
#				'ref_id' => $tx->authorization(),
#				'ref_id_2' => '0',
#				'user' => $param->{'operator'},
#				'account' => $param->{'acct_dst'},
#			});
...
#		else
#		{
#			my $errmsg = $tx->error_message();
#			$errmsg ||= 'Credit card processing failed.';
#			my $err = Cyber::Record::create('payment_error', {
#				'error_summary' => 'failure',
#				'error_data' => nfreeze({
#					'response' => $tx->server_response(),
#				}),
#				'error_text' => $errmsg,
#			});
#			$attempt->update({
...
#			'action' => 'Credit',
#			'amount' => $param->{'amount'},
#			'invoice_number' => $param->{'invoice_id'},
#			'order_number' => $attempt->{'id'}, 
#			'customer_id' => (new Cyber::Record($param->{'acct_dst'}))->data('owner')->{'id'},
#			'first_name' => $param->{'first_name'},
#			'last_name' => $param->{'last_name'},
#			'address' => $param->{'address'},
...
#			});
#			transaction(
#				'acct_src' => $param->{'acct_dst'},
#				'acct_dst' => $src,
#				'amount' => $param->{'amount'},
#				'entity' => $prec->{'id'},
#				'tx_type' => tx_type_id('refund_cc'),
#				'user' => $param->{'operator'},
#			);
#			if (defined $param->{'callback'})
#			{
