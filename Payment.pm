...
sub proc_id {
    my ($name) = @_;
    my $r = Note::Row::find_create( 'payment_proc' => { 'name' => $name, } );
    return $r->id();
}
...
sub payment_paypal {
    ...
    my $lock;
    eval { $lock = Note::Row::create( 'payment_lock', { 'account' => $dst->{'id'}, } ); };

    if ($@)    # Transaction in progress
    {
        return undef;
    }
    my $attempt = Note::Row::create(
        'payment_attempt',
        {   'amount'   => $param->{'amount'},
    ...
    $lock->update( { 'attempt' => $attempt->{'id'}, } );
    if ( dofork( $param->{'nofork'} ) ) {
        ...
        if ($@) {
            my $err = Note::Row::create(
                'payment_error',
                {   'error_summary' => 'timeout',
                    'error_data'    => nfreeze( { 'error' => $@, } ),
            ...
            if ( $resp{'Ack'} =~ /success/i ) {
                ...
                my $errdata = { 'LongMessage' => 'Unrecognized error received from payment processor.', };
                my $es      = 'error';
                my %errcode = (
                    '10002' => 'login',
                    '10504' => 'bad_cvv2',
